...
async function getProviderMetrics () {
  const token_options = { method: 'POST',
  ...
  const rows = counts.filter(p => reportProviders.includes(p.provider_id)).map(provider => {
    ...
    return { 
      ...
      deployed: sum([provider.status.available, provider.status.unavailable, provider.status.trip, provider.status.reserved]) || 0,
...
exports.handler = (event, context) => getProviderMetrics()

...
const MIGRATIONS = ['createMigrationsTable', 'alterAuditEventsColumns'] as const
...
async function alterAuditEventsColumns(exec: SqlExecuterFunction) {
  await exec(
...
async function doMigrations(client: MDSPostgresClient) {
  ...
  await doMigration(exec, 'alterAuditEventsColumns', alterAuditEventsColumns)
}
